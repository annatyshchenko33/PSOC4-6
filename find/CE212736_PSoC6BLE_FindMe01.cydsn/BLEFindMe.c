...
{      
    ...
    switch(alertLevel)
    {       
        static uint8_t previousAlertLevel = 0xFF;
        if (alertLevel != previousAlertLevel)
        {
            EinkDisplay_Update(alertLevel);
...
/* Use the MCWDT to blink the Blue LED in case of mild alert */
 ...
 /* Enable MCWDT interrupt at NVIC */   // NVIC_EnableIRQ(MCWDT_isr_cfg.intrSrc);
 ...
 /* Disable MCWDT interrupt at NVIC */ // NVIC_DisableIRQ(MCWDT_isr_cfg.intrSrc);
